...
function StockTracker(watchList) {
    ...
    this.checkAlerts = async function () {
        let result = this.watchList.filter(stock => stock.price >= stock.threshold);

        return new Promise((resolve, reject) => {
            if (result) {
                resolve(`${result} those are the list with current price greater than threshold`)
            }
            else {
...
const watchLists = [{ symbol: "A", threshold: 200, currentPrice: 500 },
{ symbol: "B", threshold: 100, currentPrice: 50 },
{ symbol: "C", threshold: 500, currentPrice: 800 }
];
const stockTrack=new StockTracker(watchLists);
stockTrack.updatePrice("B",700);
stockTrack.checkAlerts().then(msg=>console.log(msg)).catch(error=>console.log(error));